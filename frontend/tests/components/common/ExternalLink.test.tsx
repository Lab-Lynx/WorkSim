...
describe('ExternalLink', () => {
  it('renders valid https and http URLs as clickable links with safe target and rel attributes', () => {
    render(<ExternalLink href="https://github.com/worksim/repo">GitHub Repository</ExternalLink>);
    ...
    render(<ExternalLink href="http://example.com/build">CI Build Run</ExternalLink>);
  ...
  it('includes accessible screen-reader-only text for new-tab indication', () => {
    render(<ExternalLink href="https://github.com/worksim/repo">Pull Request #42</ExternalLink>);
  ...
  it('renders as plain text fallback for unsafe protocols like javascript: and data:', () => {
    const { container: jsContainer } = render(
      <ExternalLink href="javascript:alert(document.cookie)">Unsafe JS Link</ExternalLink>
    );
    ...
    const { container: dataContainer } = render(
      <ExternalLink href="data:text/html,<script>alert(1)</script>">Unsafe Data Link</ExternalLink>
    );
  ...
  it('renders plain text fallback for malformed, relative, or unsupported protocol URLs', () => {
    ...
    invalidUrls.forEach((url) => {
      const { unmount } = render(<ExternalLink href={url}>Fallback Content</ExternalLink>);
